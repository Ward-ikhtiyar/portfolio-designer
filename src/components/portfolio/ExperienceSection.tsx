  ...
  return (
    <>
      {experience.map((exp, idx) => (
        <SectionWrapper key={idx} id="experience" colors={exp.colors}>
          <SectionTitle title="Experience" primaryColor={exp.colors.primary} />
          ...
          <div className="flex items-center gap-3 mb-8">
            ...
            >
              <Briefcase className="w-5 h-5" style={{ color: `hsl(${exp.colors.primary})` }} />
            </div>
            <div>
          ...
          <MediaDisplay media={exp.media} className="mt-8" />
        </SectionWrapper>
      ))}
    </>
  );
